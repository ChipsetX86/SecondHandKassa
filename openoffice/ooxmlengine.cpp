...
bool OOXMLEngine::waitProcessEnd(QProcess* proc)
...
bool OOXMLEngine::open(QString fName, QString sheet, bool ro)
{
    ...
    if (QDir().exists(fName))
    {
        ...
        if (QDir().mkdir(tmpDir))
        {
            ...
            unzip->start(qApp->applicationDirPath() + "/unzip", QStringList() << fileName);
#endif
            qDebug() << QString(unzip->readAllStandardOutput());
            ...
            if (waitProcessEnd(unzip))
            {
               ...
               qDebug() << QString("Не удалось запустить программу unzip");
            QDir().rmdir(tmpDir);
        }
    }
    else if (showError)
        qDebug() << QString("Не удалось открыть файл");
    return result;
}
    ...
    if (!readOnly)
    {
        ...
        if (file.open(QIODevice::WriteOnly))
        {
...
#if defined(Q_OS_WIN)
            zip->start(qApp->applicationDirPath() + "/zip", QStringList() << "-r" << fileName << ".");
#else
            ...
            if (waitProcessEnd(zip))
            {
                ...
                qDebug() << QString("Не удалось запустить программу zip");
        }
    }
    else
        removeDir(tmpDir);
